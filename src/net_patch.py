...
from .libs.PatchCPG import GetDataset
...
def Train_PatchGNN(opt):
    ...
    dataset, _ = GetDataset(path=opt.np_path)     # get dataset from local dataset.
    print('[INFO] =============================================================')
...
def Test_PatchGNN(opt):
    ...
    # get parameter.
    dataset, _ = GetDataset(path=opt.tnp_path)
...
def demo_eval(opt, model):
    dataset, files = GetDataset(path=opt.np_path) if opt.task == 'train' else GetDataset(path=opt.tnp_path)
